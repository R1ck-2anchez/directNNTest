...
import torch
from ATS.AbsSelectStrategy import AbsSelectStrategy
...
class PrioritySelectStrategy(AbsSelectStrategy):
    ...
    def get_max_coverage_sequence(self, Tx, Ty, n, M, base_path=None, use_add=False, is_save_ps=False, prefix="",
        ...
        for i in tqdm(range(n)):
            ...
            if len(Tx_i) == 0:
                continue
            Tx_i = Tx_i.to(M.device)
            #print(Tx_i)
            M.eval()
            outputs = M.forward(Tx_i).cpu()
            #print("outputs", outputs)
            Tx_prob_matrixc = torch.nn.functional.softmax(outputs, dim=1)
            #print("Prob", Tx_prob_matrixc)
            #return
            #print("len:", len(Tx_prob_matrixc), "matrix", Tx_prob_matrixc)
